...
var insert = function(intervals, newInterval) {
    ...
    while(i < len && intervals[i][1] < newInterval[0]) { // 无重叠区间
        res.push(intervals[i])
        i++
    }

    while(i < len && intervals[i][0] <= newInterval[1]) { // 有重叠区间
        newInterval[0] = Math.min(newInterval[0], intervals[i][0]) // 左侧取最小值
        newInterval[1] = Math.max(newInterval[1], intervals[i][1]) // 右侧取最大值
        i++
    }
